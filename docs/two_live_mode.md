...
두 로봇 사이의 정렬은 이 정지 구간을 활용합니다. 물리 live runner는 fusion host가 각 driver의
`/r*/livox/lidar`를 직접 구독하도록 설정하므로, PC2의 mapping republisher나 원격
`/tf_static`을 기다리지 않습니다. 두 MID-360의 sensor extrinsic이 동일하다는 전제에서 cloud
좌표를 그대로 사용합니다. bag replay와 직접 `ros2 launch` 실행은 기존의 `/r*/mapping/lidar`와
sensor TF 경로를 기본으로 유지합니다. 각 입력은 서로를 기다리지 않고, 자기 입력이 처음 보인 뒤
